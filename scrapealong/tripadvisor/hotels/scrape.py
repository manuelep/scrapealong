...
def details(response,link):
    ...
    try:
        ...
        about=response.findAll("div",
                      attrs={"class":"ui_columns _318JyS8B"})

        about=[x for x in about if "Property amenities" in x.text][0]

        about_ui=about.findAll("div",{"class":"ui_column"})
        about_ui=[x for x in about_ui if "Property amenities" in x.text][0]
